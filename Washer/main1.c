...
void SplitTimeAndType (int amountOfItems, char** lines, char*** tokens, int* tokensCount, int* timeForSuchWare, char** typeOfItem)
{
    for (int i = 0; i < amountOfItems; i++)
    {
        ...
        timeForSuchWare[i] = atof(tokens[i][1]);
...
int main()
{
    ...
    FILE* timeToWipeForEachType = fopen("/home/ivan/Programming/Washer/TimeToWipeForEachType", "r");
    FileInit(timeToWipeForEachType);
    int amountOfTypesOfWare = ScanFileAndReturnNumberOfLines(&lines, timeToWipeForEachType);
    fclose(timeToWipeForEachType);
    ...
    for (int i = 0; i < amountOfTypesOfItems; i++)
    {
        ...
        {
		if(*numberOfItemsOnTable != 0)
		{
		    printf("%d\n", *numberOfItemsOnTable);
                    sleep(timesForSuchTypeOfItem[i]);
                    int end = time(NULL);
                    printf("Wiped %s, wiping tooked %f seconds\n", typeOfItem[i], difftime(end, start));
